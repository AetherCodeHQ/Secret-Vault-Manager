...
> Secret-Vault-Manager - Open source tool by AetherCodeHQ

`cli` `golang` `secrets` `security` `vault`

## What is it?

**Secret Vault Manager** is a security tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./Secret-Vault-Manager --path ./target

# With options
./Secret-Vault-Manager --path ./target --format json --output report.json

# Verbose mode
./Secret-Vault-Manager --path ./target --verbose
```
...
### Basic scan
```bash
./Secret-Vault-Manager --path ./my-project
```

### JSON report
```bash
./Secret-Vault-Manager --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Secret Vault Manager
  run: ./Secret-Vault-Manager --path . --format json --output report.json
```
